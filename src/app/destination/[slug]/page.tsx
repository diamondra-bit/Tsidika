...
export default function DestinationDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [nextDest, setNextDest] = useState<any | null>(null);
    ...
    async function loadDestinationData() {
      try {
        ...
        const { data: nextData } = await supabase
          .from('destinations')
          .select('name, slug, image_main')
          .neq('slug', resolvedParams.slug)
          .limit(1)
          .maybeSingle();
        
        if (nextData) setNextDest(nextData);
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FDFCFB] flex items-center justify-center italic text-slate-400 font-serif">
  ...
  return (
    <div className="bg-[#FDFCFB] min-h-screen font-lato selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <header className="relative h-[65vh] w-full flex items-end overflow-hidden bg-[#FDFCFB]">
        <Image
          src={dest.hero_image || dest.image_main} 
          alt={dest.name}
          fill
          priority
          className="object-cover object-center scale-105"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#FDFCFB] via-[#FDFCFB]/50 to-transparent" />
            ...
            <div className="lg:col-span-7 space-y-16">
              ...
              <div className="space-y-8 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-12">
                  {dest.points_forts?.map((pt: any, i: number) => (
                ...
                <div className="space-y-8">
                  <div className="flex gap-5 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-serif text-slate-800 leading-tight">{dest.parent_region}</p>
                    </div>
                  </div>
                  <div className="flex gap-5 items-start">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          {activeTab === 'hotels' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {dest.hebergements?.map((hotel: any, i: number) => (
          ...
          {activeTab === 'restos' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {dest.gastronomie?.map((resto: any, i: number) => (
                <div key={i} onClick={() => { setSelectedItem(resto); setItemType('resto'); }} className="cursor-pointer">
                  <DestinationCard 
                    type="resto" 
                    title={resto.name} 
                    image={resto.img} 
                    priceOrBudget={`${resto.budget_min}€ - ${resto.budget_max}€`} 
                    stars={resto.stars} 
                  />
                </div>
              ))}
          ...
          {activeTab === 'activities' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {dest.activites?.map((activity: any, i: number) => (
                <div key={i} onClick={() => { setSelectedItem(activity); setItemType('activity'); }} className="cursor-pointer">
                  <ActivityCard 
